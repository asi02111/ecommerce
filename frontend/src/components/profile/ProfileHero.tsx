      ...
      <Box sx={{ maxWidth: '1152px', mx: 'auto', px: { xs: 2, md: 4 }, pt: 5 }}>
        <Paper elevation={0} sx={{ borderRadius: 4, overflow: 'hidden' }}>
          ...
          <Box sx={{ px: { xs: 3, md: 5 }, pb: 4, mt: -7, display: 'flex', flexDirection: { xs: 'column', md: 'row' }, alignItems: { xs: 'center', md: 'flex-end' }, gap: { xs: 2, md: 4 } }}>
            ...
            {/* Stats */}
            <Stack sx={{ direction: "row", spacing: 1.5, flexWrap: "wrap", justifyContent: { xs: "center", md: "flex-end" }, mt: { xs: 2, md: 0 } }}>
              {stats.map((s) => (
                <Paper
                  key={s.label}
                  elevation={0}
                  sx={{ width: 88, height: 88, border: '1.5px solid #eef2ff', borderRadius: 3, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 0.3, transition: '.2s', cursor: 'default', '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 10px 28px rgba(79,70,229,.12)', borderColor: '#c7d2fe' } }}
                >
                  <Box color="primary.main" sx={{ display: 'flex' }}>{s.icon}</Box>
                  <Typography sx={{ fontWeight: 800, fontSize: 16 }}>{s.value}</Typography>
